...
void free_mem_solutions(Solutions *solucions) {
    for (int i = 0; i < MAX_SOLUTIONS; i++) {
        free(solucions->solution[i]);
    }
}

void free_mem(WorldData *dades, Solutions *solucions, Solutions *fills, Solutions *solucions2, Solutions *fills2) {
    free(dades->cities);
    free_mem_solutions(solucions);
    free_mem_solutions(fills);
    free_mem_solutions(solucions2);
    free_mem_solutions(fills2);
}

void load_data(WorldData *dades) {
    printf("How many cities are in the map? ");
    scanf("%d", &dades->total_cities);

    dades->cities = (City *) malloc(dades->total_cities * sizeof(City));
    for (int i = 0; i < dades->total_cities; i++) {
        printf("\nCoordinate X of city %d:", i);
        scanf("%d", &dades->cities[i].x);
        printf("\nCoordinate Y of city %d:", i);
        scanf("%d", &dades->cities[i].y);
    }
}
...
void init_solutions(int total_cities, Solutions *solutions, Solutions *solutions_inheritors) {
    ...
    // Initialize solutions with cities numbers sequentially
...
uint32_t get_distance(int nodea, int nodeb, WorldData *dades) {
    return (uint32_t)floor(sqrt(pow((dades->cities[nodea].x) - (dades->cities[nodeb].x), 2) +
                                pow((dades->cities[nodea].y) - (dades->cities[nodeb].y), 2)));
}

int get_city_index_in_solution(int city_number, const uint32_t *solucio) {
    int i = 0;
    while (1) {
        if (solucio[i] == city_number) {
            return i;
        } else i++;
    }
}
...
void evaluate_solutions(Solutions *solutions, WorldData *dades) {
    int d;
    for (int i = 0; i < MAX_SOLUTIONS; i++) {
        solutions->score[i] = 0;
        for (int j = 1; j < dades->total_cities; j++) {
            d = get_distance(solutions->solution[i][j - 1], solutions->solution[i][j], dades);
            solutions->score[i] = solutions->score[i] + d;
        }
        d = get_distance(solutions->solution[i][0], solutions->solution[i][dades->total_cities - 1], dades);
        solutions->score[i] = solutions->score[i] + d;
    }
}
...
uint32_t get_best_neighbour(int node, uint32_t *neighbours, WorldData *data) {
    uint32_t distance, n, best_neighbour = UNDEFINED, best_distance = UINT32_MAX; //aquesta �s una distance�ncia impossible
    int i = 0;
    ...
    if (n != UNDEFINED)
        ...
        while (neighbours[i] != UNDEFINED) {
            distance = get_distance(node, neighbours[i], data);
...
void fill_with_unused_cities(uint32_t *neighbours, const bool *cities_used, int num_nodes) {
    int p = 0;
...
void mutate_solution(uint32_t *solution, int total_cities) {
    int x, y;
    uint32_t tmp;
    div_t e = div(total_cities, 10);        //per cada 10 cities farem una mutaci�
...
void crossover_and_mutation(Solutions *solutions, Solutions *inheritors, WorldData *data) {
    for (int i = 0; i < MAX_SOLUTIONS-1; i++) {
        ...
        while ((pos < total_cities) && !end) {
            get_neighbours(x, solutions->solution[i], solutions->solution[i + 1], cities_used, total_cities, neighbours, pos);
...
void print_solutions(Solutions *solucions, int total_citites) {
    for (int i = 0; i < MAX_SOLUTIONS; i++) {
        for (int e = 0; e < total_citites; e++) {
            printf("%d ", solucions->solution[i][e]);
        }

        printf("-> %d%s\n", solucions->score[i], i == 0 ? " <- BEST SOLUTION" : "");
    }
}
...
void shift_solutions(Solutions *pares, int i, int num_nodes) {
    if (i == MAX_SOLUTIONS-1) {}
    else {
        for (int e = MAX_SOLUTIONS-2; e >= i; e--) {
            copy_solution(pares->solution[e + 1], pares->solution[e], num_nodes);
...
void draw(uint32_t best_solution[], uint32_t score, RenderTexture2D *render_texture, float scale, WorldData *data) {
    ...
    for(int i=0; i<data->total_cities; i++) {
        int current_city = best_solution[pos(i, data->total_cities)];
        int next_city = best_solution[pos(i+1, data->total_cities)];
        DrawLine(data->cities[current_city].x * scale, data->cities[current_city].y * scale, data->cities[next_city].x * scale , data->cities[next_city].y * scale, DARKBLUE);
    }
    ...
    EndTextureMode();
    DrawTexturePro(render_texture->texture, (Rectangle){ 0, 0, render_texture->texture.width, -render_texture->texture.height }, (Rectangle){ 0, 0, 2*SCREEN_WIDTH, 2*SCREEN_HEIGHT }, (Vector2){ 0.0f, 0.0f }, 0, WHITE);
    EndDrawing();
}

void init_window(const char title[], int width, int height, RenderTexture2D *render_texture, float *scale, WorldData *data) {
    InitWindow(width, height, title);
    //SetWindowPosition(676, 156);
    //SetTargetFPS(60);
